### Alameda-Specific Deployment Instructions

1. Be sure you're in the repo root
2. `npm install`
4. `npm run build`
5. `npm run cdk deploy -- --parameters adminUserEmail=<ADMIN_EMAIL_ADDRESS>`
6. Set the variables at the top of `deployment/deploy.sh`. (Both should be found in the output variables of the CloudFormation stack.)
  - `INPUT_BUCKET_NAME`
  - `STEP_ARN`
7. Run:
```bash
...
```
8. Use the following to monitor the ingestion Step Function state:
```bash
...
Set the variables at the top of `deployment/deploy.sh`:
- `INPUT_BUCKET_NAME`
- `STEP_ARN`
Both should be found in the output variables of the CloudFormation stack.
...
Once files are ingested, this outputs a Step Function execution ARN. Use the following to monitor the ingestion Step Function state:
